...
async function routes(fastify: FastifyInstance) {
    ...
    fastify.get<{ Params: RouteParams }>('/posts/:id', async (request, reply) => {
        const [rows]:any = await fastify.db.query('SELECT * FROM posts WHERE id = ?', [request.params.id]);
        if (rows.length === 0) {
          reply.status(404);
      ...
      fastify.put<{ Params: RouteParams }>('/posts/vote/:id', async (request, reply) => {
        ...
        if (rows.length === 0) {
          ...
          if (selectedClick.selectedClick === 1) {
            await fastify.db.query('UPDATE posts SET nb_clic1 = nb_clic1 + 1 WHERE id = ?', [request.params.id]);
          } else if (selectedClick.selectedClick === 2) {
            await fastify.db.query('UPDATE posts SET nb_clic2 = nb_clic2 + 1 WHERE id = ?', [request.params.id]);
          } else {
            reply.status(400);
